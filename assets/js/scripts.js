let currid = 0;
//Funcion para añadir las actividades en listas
function myFunction() {
    let input = ''
    let filter = ''
    let table = ''
...
//Se inicia la busqueda por nombre o actividad
...
//Añadir y Borrado de actividades
function limpiar() {
    document
...
function salvar(evento) {
    ...
    if (evento[0] !== "" && evento[1] !== "" && evento[2] !== "") {
        ...
        window
            .localStorage
            .setItem(n, JSON.stringify(objeto))
...
function terminar(ev) {
    ...
    window
        .localStorage
        .setItem(id, JSON.stringify(objeto))
    currid = 0;
    location.reload()
}
...
'<p class="w3-medium">Aplicacion CRUD lista de tareas para ucamp, por Jose Manuel Silva Sereno... sí, asi es... tuve que estudiarlo en W3 SCHOOLS jajajaja</a></p>' +
'</footer>'
